...
```{r paged.print=FALSE}
library(forcats)
NHANES$AgeDecade <- fct_explicit_na(NHANES$AgeDecade, na_level = "MissingData")
NHANES %>%
...
In Section [4.5](https://rafalab.github.io/dsbook/programming-basics.html#vectorization) we learned about the `sapply` function, which permitted us to apply the same function to each element of a vector. We constructed this function:
```{r}
...
A typical data analysis will often involve one or more conditional operation. In Section [4.1](https://rafalab.github.io/dsbook/programming-basics.html#conditionals) we described the `ifelse` function, which we will use extensively in this book. In this section we present two **dplyr** functions that provide further functionality for performing conditional operations.
