...
```{r}
ma <- data.frame(name = a, 
                 is_a_positive = ifelse(a>0, TRUE, FALSE),
                 answer1 = 1/a,
                 answer2 = NA,
                 result = ifelse(a > 0, 1/a, NA)
)
...
For more on this more advanced topic we recommend the R packages book^[16](http://r-pkgs.had.co.nz/namespace.html)^.
...
a. `all(x)` : every entry = true -> true
b. `any(x)` : any entry = true -> true
c. `any(!x)` : every entry = true -> false
d. `all(!x)` : any entry = true -> false
...
Setting x value in my_func doesn't work until running my_func.
