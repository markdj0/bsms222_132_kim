...
![](https://rafalab.github.io/dsbook/dataviz/img/fig1e.png)
...
Redefine he `state` object so that the levels are re-ordered. Print the new object `state` and its levels so you can see that the vector is not re-ordered by the levels.
...
avg <- us_contagious_diseases %>%
  ...
  group_by(year) %>%
  summarise(SP_rate = sum(count, na.rm = TRUE)/sum(population, na.rm = TRUE) * 10000)
...
us_contagious_diseases %>%
  ...
  geom_line(aes(year, rate, group = state), color = "grey50",
            show.legend = FALSE, alpha = 0.2, size = 1) +
  geom_line(mapping = aes(year, SP_rate), data = avg, size = 1) +
...
us_contagious_diseases %>%
  ...
  geom_line(size = 1) +
  scale_y_continuous(trans = 'sqrt') +
  ggtitle("Disease cases per 10,000 in California")
...
us_contagious_diseases %>%
  ...
  geom_line(size = 1) + 
  scale_y_continuous(trans = 'sqrt') +
  ggtitle("Disease cases per 10,000 in US")
```
