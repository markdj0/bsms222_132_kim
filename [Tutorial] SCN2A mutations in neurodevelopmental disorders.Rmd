...
Here we obtain the list of mutations in the **Supplementary Table 1** from [Sanders et al. (2018)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6015533/). 
...
**A: I mean... almost everything...? Except Chr & Recurrence's'**
...
```
**Ohhhhhh...!
Every column has been changed into character!**
...
![exon1](https://ars.els-cdn.com/content/image/1-s2.0-S1525157810603546-gr2.gif)
...
If you need more information on SCN2A, please visit the [Uniprot description](https://www.uniprot.org/uniprot/Q99250) for SCN2A. The Uniprot database contains description for protein domains. 
...
```{r}
d %>% 
  mutate(Pnum = gsub('[^0-9]', '', d[1:293, 'p.Protein'] ))
```
...
```{r paged.print=FALSE}
# Which variants are occurred in multiple times?
library(dplyr)
fd <- d %>%
  group_by(p.Protein) %>%
  count() %>%
...
# Which columns you would use for more grouping?
d %>% group_by(Alt) %>% count() # I'll use Alt

# If you find that, please check the recurrent mutations for each group.
fAlt <- d %>% group_by(Alt) %>% count() %>% filter(n>1)
nrow(fAlt)
```
...
```{r paged.print=FALSE}
phe <- d %>% 
  filter(Effect %in% c("Missense", "Nonsense", "Frameshift")) %>%
  group_by(Classification) %>% 
  count() %>% mutate(proportion = n / nrow(d))
phe
d %>% group_by(Classification) %>% count() %>% mutate(proportion = n / nrow(d))
```
...
```{r paged.print=FALSE}
# Another question you can ask is whether different mutation consequences occur in each phenotype.
dmc <- d %>%
  filter(Effect%in%c("Missense","Nonsense","Frameshift"))%>%
  group_by(Classification, Effect) %>%
  count()
dmc
# Let's find out how many mutation consequences are observed in each phenotype. 
table(dmc$Classification)
```
...
```{r paged.print=FALSE}
d$numDNA <- gsub('[^0-9]', '', d[1:293, 'c.DNA'] )
d %>%
  group_by(numDNA, Effect) %>%
  count() %>%
  filter(n>1)
```
...
```{r}
age <- d %>%
  mutate(P.age = gsub('[~><]', '', d[1:293, 'PatientAgeAtAssessment'] )) %>%
  group_by(P.age, Type) %>%
  count()
```

![visual](https://i.imgur.com/cO1rPcc.png)