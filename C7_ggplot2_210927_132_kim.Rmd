...
Throughout the book, we will be creating plots using the [ggplot2](http://ggplot2.org/) package.
```{r}
library(dplyr)
library(ggplot2)
```
...
The first step in learning **ggplot2** is to be able to break a graph apart into components. Let<e2>??s break down the plot above and introduce some of the **ggplot2** terminology. The main three components to note are:
...
![](https://rafalab.github.io/dsbook/R/img/ggplot2-cheatsheeta.png)
![](https://rafalab.github.io/dsbook/R/img/ggplot2-cheatsheetb.png)
...
This is preferred as it makes it easier to read the text. In Section [7.11](https://rafalab.github.io/dsbook/ggplot2.html#add-on-packages) we learn a better way of assuring we can see the points and the labels.
...
We can change the line type and color of the lines using arguments. Also, we draw it first so it doesn<e2>??t go over our points.
```{r}
