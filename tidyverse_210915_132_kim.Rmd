...
```{r paged.print=FALSE}
#is.na -> 결측값 확인, na.rm=true -> na를 remove. 결측값 제외
ref <- NHANES %>%
...
### 4.11.5 Create a tibble using `data_frame` instead of `data.frame`
...
A typical data analysis will often involve one or more conditional operation. In Section [3.1](https://rafalab.github.io/dsbook/programming-basics.html#conditionals) we described the `ifelse` function, which we will use extensively in this book. In this section we present two **dplyr** functions that provide further functionality for performing conditional operations.
...
**Answer: b**
Tibble is much more simplified version of data frame.
