---
title: "programmingbasics_20190916_132_kim"
output: html_notebook
---

* [rbasic_20190906_132_kim](rbasic_20190906_132_kim.nb.html)
* [rbasic_20190909_132_kim](rbasic_20190909_132_kim.nb.html)
* [programmingbasics_20190911_132_kim](programmingbasics_20190911_132_kim.nb.html)
...
How can we compute $S_n$ for various values of $n$, say $n = 1,\ldots,25$? Do we write 25 lines of code calling `compute_s_n`?  No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of $n$. For-loops let us define the range that our variable takes (in our example $n = 1,\ldots,10$), then change the value and evaluate expression as you _loop_.
...
```{r}
m <- 25
s_n <- vector(length = m) # _create an empty vector_
for(n in 1:m){
  s_n[n] <- compute_s_n(n)
}
```
...
```
! -> reverse
A. all(x) -> If every entry is true, then true. But if at least one is false, then false.
B. any(x) -> If any entry is true, then true.
C. any(!x) -> If any entry is true, then false. But if at least one is false, then true.
D. all(!x) -> If any entry is true, then false.
```
...
s_n
```
**제발 install 이후에 library 입력해서 부착을 하자**
